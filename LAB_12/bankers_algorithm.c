...
void executeProcess (int pid) {
	printf("Executed process %d, now free [", pid);
	for (i = 0; i < m; ++i) {
		AVAIL[i] += ALLOC[pid][i];
		printf(" %d", AVAIL[i]);
	}
...
void executeBankers () {
	...
	do {
		...
		if ((pid = findProcessToExecute()) != -1) {
			executeProcess(pid);
			executecount += 1;
		}

		if (executecount == n) {
			printf("\n\nALL PROCESSES EXECUTED! SYSTEM IN SAFE STATE!");
			break;
		}
	...
	if (executecount != n) {
		printf("\n\nNOT ALL PROCESSES EXECUTED; SYSTEM IN UNSAFE STATE!");
	}
